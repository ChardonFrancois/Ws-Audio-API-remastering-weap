...
;(function (OpusApplication) {
  ...
  OpusApplication[(OpusApplication['RestrictedLowDelay'] = 2051)] =
    'RestrictedLowDelay'
...
let OpusEncoder = (function () {
  function OpusEncoder(sampling_rate, channels, app, frame_duration) {
    ...
    this.in_off = 0
    this.out_ptr = 0
    if (!Opus.validFrameDuration(frame_duration)) throw 'invalid frame duration'
    ...
    this.in_i16 = HEAP16.subarray(
      this.in_ptr >> 1,
      (this.in_ptr >> 1) + this.in_len
    )
    this.in_f32 = HEAPF32.subarray(
      this.in_ptr >> 2,
      (this.in_ptr >> 2) + this.in_len
    )
  ...
  OpusEncoder.prototype.encode = function (pcm) {
    ...
    while (pcm.length - pcm_off >= this.in_len - this.in_off) {
      if (this.in_off > 0) {
        this.in_i16.set(
          pcm.subarray(pcm_off, pcm_off + this.in_len - this.in_off),
          this.in_off
        )
      ...
      if (ret <= 0) throw 'opus_encode failed: ' + ret
  ...
  OpusEncoder.prototype.encode_final = function () {
    if (this.in_off == 0) return new ArrayBuffer(0)
    for (var i = this.in_off; i < this.in_len; ++i) this.in_i16[i] = 0
    var ret = _opus_encode(
      this.handle,
      this.in_ptr,
      this.frame_size,
  ...
  OpusEncoder.prototype.encode_float_final = function () {
    if (this.in_off == 0) return new ArrayBuffer(0)
    for (var i = this.in_off; i < this.in_len; ++i) this.in_f32[i] = 0
    var ret = _opus_encode_float(
      this.handle,
      this.in_ptr,
      this.frame_size,
...
var OpusDecoder = (function () {
  function OpusDecoder(sampling_rate, channels) {
    ...
    this.in_buf = HEAPU8.subarray(
      this.in_ptr,
      this.in_ptr + Opus.getMaxFrameSize(channels)
    )
    ...
    this.out_i16 = HEAP16.subarray(
      this.out_ptr >> 1,
      (this.out_ptr + out_bytes) >> 1
    )
    this.out_f32 = HEAPF32.subarray(
      this.out_ptr >> 2,
      (this.out_ptr + out_bytes) >> 2
    )
  }
  OpusDecoder.prototype.decode = function (packet) {
    this.in_buf.set(new Uint8Array(packet))
    var ret = _opus_decode(
      this.handle,
      this.in_ptr,
      packet.byteLength,
      this.out_ptr,
    ...
    if (ret < 0) throw 'opus_decode failed: ' + ret
